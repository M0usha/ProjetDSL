	...
	override void doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {
		...
		def generateInstruction(Instruction instruction) '''
				«IF (instruction.cmd !== null)»
					...
					switch(cmd){
						case "url":
							driver.get("«instruction.param»");
						break;
						
						case "click":
							«IF (instruction.obj !== null)»
								
								obj = "«instruction.obj»";
								
								switch(obj) {
									case "link":
										WebElement elem = driver.findElement(By.xpath("//a[contains(text(),'«instruction.param»')]"));
										elem.click();
									break;	
								}
							«ENDIF»
		...
		def generateCommande(Instruction instruction, String string) '''
		...
		def generateVerify(Verify verify) '''
			...
			«IF (verify.contains !== null)»
				String objet = "«verify.contains.obj»";
				switch(objet){
					case "link":
					WebElement elem = driver.findElement(By.xpath("//a[contains(text(),'«verify.contains.value»')]"));
					boolean testContains = (elem != null );
		...
		def generateTab(Tab tab) '''
			...
			List<String> list«tab.v.name» = new ArrayList<String>();
			«FOR value : tab.values»
				liste«tab.v.name».add("«value»");
			«ENDFOR»
